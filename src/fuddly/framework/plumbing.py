    ...
    def _import_project(self, prefix, name, reload_prj=False):
        ...
        try: 
            if type(prefix) is importlib.metadata.EntryPoint: # Reloading from a python module
                ...
                load_from_module=True
                module = prefix.load()
                # TODO check if this reload is needed (if it is, it is missing in import_dm):w
                if reload_prj:
            ...
            if reload_prj:
                # TODO Adapt error message to include files and modules
        ...
        try:
            ...
            if not self._quiet:
                # TODO make error message work with modules and files
                self.print(colorize("*** ERROR: '%s.py' shall contain a global variable 'project' ***" % (name), rgb=Color.ERROR))
            return None
